...
/**
 * This class extends the Player class to include decision-making capabilities
 * for the AI in Single Player gameplay mode. Attributes: skillLevel: SkillLevel
 * Methods: AI() AI(SkillLevel) checkBlockOpponent(Stone[][]): Location
 * checkBlockOpponentFork(Stone[][]): Location checkCorners(Stone[][]): Location
 * checkFork(Stone[][]): Location checkFork(Stone[][], Color): Location
 * checkOppositeCorner(Stone[][]): Location checkSides(Stone[][]): Location
 * checkScorePoint(Stone[][]): Location determineNextEasyMove(Stone[][]):
 * Location determineNextHardMove(Stone[][]): Location
 * determineNextMediumMove(Stone[][]): Location getCurrentGridPanel: GridPanel
 * getSkillLevel(): skillLevel makeMove(): void
 *
 * @author Jonathan
 * @author Alice
 * @version 1.0
...
public class AI extends Player
{
	 ...
	 * decision-making function will be called to determine the next move of the
	 * AI.
	 *
	 * @see SkillLevel
	 */
	 ...
	 * appropriate function to determine the next (Easy-, Medium-, or Hard-mode)
	 * AI move. Passes move location to makeMove(Location) to be placed on the
	 * board.
	 *
	...
	/**
	 * Determines next move when skillLevel is set to Easy (Easy-mode). Passes
	 * the current game board to each move-checking function until a non-null
	 * location is returned (a valid move is found) in the Easy-mode priority
	 * order.
	 *
...
//            }
	...
	/**
	 * Checks whether there is an empty spot on any side of the board, excluding
	 * corners.
	 *
	 * @param grid the current game board
	 *
	 * @return location of an empty side spot. If there are no empty side spots,
	 *         returns null.
	 */
	 ...
	 * @param grid the current game board
	 *
	 * @return location of an empty corner spot. If there are no empty corner
	 *         spots, returns null.
	 */
	...
	/**
	 * Checks whether there is an empty corner spot opposite from a corner spot
	 * occupied by the opponent.
	 *
	 * @param grid the current game board
	 *
	 * @return location of an empty corner spot which is opposite to a corner
	 *         occupied by the opponent. If there is no such spot, returns null.
	 */
	...
	/**
	 * Checks whether a point can be scored by the AI. Looks for configurations
	 * of 3, 4, or 5 stones in a row which can be extended to score a point.
	 *
	 * @param grid the current game board
	 *
	 * @return location of a spot which will complete a scoring configuration.
	 *         If there is no such spot, returns null.
	 */
	 ...
	 * checkScorePoint, looks for scoring configurations, but in the opponent's
	 * color.
	 *
	 * @param grid the current game board
	 *
	 * @return location of a spot which will block the opponent from scoring a
	 *         point by occupying a spot the opponent needs to complete a
	 *         scoring configuration. If there is no such spot, returns null.
	...
	/**
	 * Checks whether the AI can fork (create two different potential scoring
	 * configurations on the board). Calls the auxiliary function
	 * checkFork(Stone[][], Color) with the AI's color.
	 *
	 * @param grid the current game board
	 *
	 * @return location of a spot which will create a fork. If there is no such
	 *         spot, returns null.
	...
	/**
	 * Checks whether the player with the color playerColor can fork (create two
	 * different potential scoring configurations on the board). Auxiliary for
	 * checkFork(Stone[][]) and checkBlockOpponentFork(Stone[][]). Creates a
	 * dummy copy of the game board and checks every empty spot by placing a
	 * stone and counting scoring configurations. A forking location is found if
	 * a stone placement results in at least 2 scoring configurations on the
	 * board.
	 *
	 * @param grid        the current game board
	 ...
	 * scoring configurations on the board). Similarly to checkFork(Stone[][]),
	 * calls the auxiliary checkFork(Stone[][], Color) function, but with the
	 * opponent's color.
	 *
	 * @param grid the current game board
	 *
	 * @return location of a spot which will block a forking opportunity for the
	 *         opponent by occupying a spot the opponent needs to create a fork.
	 *         If there is no such spot, returns null.
	 * @see #checkFork(Stone[][], java.awt.Color)
	 */
	 ...
	 *
	 * @return the current gridPanel instance. If the current panel is not an
	 *         instance of gridPanel (that is, there is no game currently in
	...
	/**
	 * Counts all three in a rows.
	 *
	 * @param grid  The grid to perform the calculations on.
	 * @param color The color to look for three in a rows for.
	 *
	 * @return The number of three in a rows.
	 */
